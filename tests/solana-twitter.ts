...
describe('solana-twitter', () => {
  ...
  it('can send a new tweet from a different author', async () => {
    ...
    // Call the "SendTweet" instruction.
    try {
      await program.rpc.sendTweet('choose', 'stuck in my new rust project..', {
        accounts: {
          // Accounts here...
          tweet: tweet.publicKey,
  ...
  it('can fetch all tweets', async () => {
    ...
    // making sure that we have created 3 accounts with all our tests
  ...
  it('can filter tweets by author', async () => {
    ...
    const tweetAccounts = await program.account.tweet.all([{
      memcmp: {
        ...
        bytes: authorPublicKey.toBase58(), // My base-58 encoded public key.
      }
    }]);
  ...
  it('can filter tweets by topics', async () => {
    const tweetAccounts = await program.account.tweet.all([{
      memcmp: {
        ...
        bytes: bs58.encode(Buffer.from('choose')),
      }
    }]);
    assert.equal(tweetAccounts.length, 2);
    assert.ok(tweetAccounts.every(tweetAccount => {
      return tweetAccount.account.topic === 'choose'
    }));
  });
});
